...
class Hero(Character):
    ...
    def look(self, direction=None):
        ...
        return self._game_map.tiles(self._current_position, direction)
    ...
    def has_plans(self):
        return self._plans and len(self._plans) > 0

    def act(self):
        self.action()

        if self._has_action and self._plans and len(self._plans) > 0:
            action = self._plans.pop(0)
            action()
...
class GameMap(object):
    ...
    def tiles(self, position, direction):
        if not self.check_position(position):
            return ['wall']
        result = self.tiles(self._adjacent_position(position, direction), direction)
        result.insert(0, self._get_named_version(self._adjacent_position(position, direction)))
        return result

    def _get_named_version(self, position):
        return {
            'z': 'hero',
            'w': 'wall',
            'm': 'monster',
    ...
    def _adjacent_position(self, position, direction):
        if direction == 'up':
